		...
		if (ticker)
			...
			if(ticker.mode.name == "AI malfunction" && ticker.processing)
				if(ticker.AItime >= 6000)
					stat(null, text("Time until AI takes over all SS13's systems: [(ticker.AIwin - ticker.AItime) / 600 % 60]:[(ticker.AIwin - ticker.AItime) / 100 % 6][(ticker.AIwin - ticker.AItime) / 10 % 10]"))
		if (src.internal)
			if (!( src.internal.gas ))
				//src.internal = null
				del(src.internal)
...
/mob/human/UpdateClothing()
	...
	if (src.ishulk) //MUTMARKER1 //add this to monkeys
		src.overlays += image("icon" = 'genetics.dmi', "icon_state" = "hulk[!src.lying ? "_s" : "_l"]")

	if (src.jacksparrow) //MUTMARKER1 //add this to monkeys
		src.overlays += image("icon" = 'traps.dmi', "icon_state" = "jacksparrow[!src.lying ? "_s" : "_l"]")

	if (src.firemut) //MUTMARKER1 //add this to monkeys
		src.overlays += image("icon" = 'genetics.dmi', "icon_state" = "fire[!src.lying ? "_s" : "_l"]")

	if (src.telekinesis) //MUTMARKER2 //add this to monkeys
		src.overlays += image("icon" = 'genetics.dmi', "icon_state" = "telekinesishead[!src.lying ? "_s" : "_l"]")
	...
	if ((src.wear_mask && !(src.wear_mask.see_face)) || (src.head && !(src.head.see_face))) // can't see the face
		...
		if (src.wear_id && src.wear_id.registered != src.rname)
			src.name = "[src.rname] (as [src.wear_id.registered])"
		else
			src.name = src.rname
...
/*		if (!src.r_hand.blood)
			...
			var/icon/I = new /icon("icon" = 'items_in_hand.dmi', "dir" = WEST, "icon_state" = src.r_hand.s_istate ? src.r_hand.s_istate : src.r_hand.icon_state)
			I.Blend(new /icon('blood.dmi', "thisisfuckingstupid"),ICON_ADD)
			I.Blend(new /icon('blood.dmi', "handblood"),ICON_MULTIPLY)
			I.Blend(new /icon(src.icon, src.icon_state),ICON_UNDERLAY)
			src.overlays += image(I, "layer" = MOB_LAYER)
			I = null
*/
...
/*		if (!src.l_hand.blood)
			...
			var/icon/I = new /icon("icon" = 'items_in_hand.dmi', "dir" = EAST, "icon_state" = src.l_hand.s_istate ? src.l_hand.s_istate : src.l_hand.icon_state)
			I.Blend(new /icon('blood.dmi', "thisisfuckingstupid"),ICON_ADD)
			I.Blend(new /icon('blood.dmi', "handblood"),ICON_MULTIPLY)
			I.Blend(new /icon(src.icon, src.icon_state),ICON_UNDERLAY)
			src.overlays += image(I, "layer" = MOB_LAYER)
...
/mob/human/attack_hand(mob/human/M as mob)
	...
	if (M.gloves && M.gloves.elecgen == 1 && M.a_intent == "hurt")
		if(M.gloves.uses > 0)
			M.gloves.uses--
			if (src.weakened < 5)
				src.weakened = 5
...
/obj/equip_e/human/done()
	...
	switch(src.place)
		...
		if("drink")
			...
			var/a = S.name
			S.ingest(src.target)
			src.target << "\blue <B>You take a swig of SPACE BEER!</b>"
			for(var/mob/O in viewers(src.source, null))
